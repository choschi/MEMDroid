...
public class ResultFactory {
	...
	public static Result factory (Object soapResponse){
		...
		Result result = null;
		try{
			result = ResultFactory.handlePrimitive((SoapPrimitive) soapResponse);
		}catch (Exception ex){Log.d(TAG,"not a primitive: "+ex.getMessage());}
		try{
			result = ResultFactory.handleObject((SoapObject) soapResponse);
		}catch (Exception ex){Log.d(TAG,"not an object "+ex.getMessage());}
		try{
			result = ResultFactory.handleFault((SoapFault) soapResponse);
		}catch (Exception ex){
			Log.d(TAG,"not a fault "+ex.getMessage());
		}
		try{
			result = ResultFactory.handleException ((Exception) soapResponse);
		}catch (Exception ex){
